/**
 * 🪶 Archaeologist AI Artefact Analyzer (No OAuth)
 */

const GEMINI_API_KEY = 'AIzaSyDJz6-OLwZohEnd9Ty5fmj9eEVe42ed8-g'; // replace with your Gemini API key
...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'getUserProfile') {
      ...
      if (!userEmail) return jsonError('User not recognized', 401);
      const aiText = analyzeArtifact(body.form || {}, body.base64Image || null, userEmail);
      return ContentService.createTextOutput(JSON.stringify({ aiText: aiText }))
        .setMimeType(ContentService.MimeType.JSON);
      ...
      return ContentService.createTextOutput(JSON.stringify({ analyses: analyses }))
        .setMimeType(ContentService.MimeType.JSON);
...
/** Analyze artefact using Gemini and save result */
...
/** Get past analyses for signed-in user */
function getUserAnalyses(userEmail) {
  const sheet = getOrCreateSheet(SHEET_NAME);
  const rows = sheet.getDataRange().getValues();
  const userRows = rows.filter((row, i) => i > 0 && row[0] === userEmail);
